...
func NewCertManagerSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc: GenerateCertManager, UseWorkloadManifest: true,
		WorkloadKind: "CertManager",
	}

	generateCmd.Setup()
}

// GenerateCertManager runs the logic to generate child resources for a
// CertManager workload.
func GenerateCertManager(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1certmanager.GenerateForCLI,
