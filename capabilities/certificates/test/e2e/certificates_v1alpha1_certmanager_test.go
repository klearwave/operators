...
import (
	...
	"github.com/klearwave/operators-capability/capabilities/certificates/apis/certificates/v1alpha1/certmanager"
)

// certificatesv1alpha1CertManager tests
func certificatesv1alpha1CertManagerChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(certmanager.CreateFuncs) == 0 {
		return nil
	}

	workload, err := certmanager.ConvertWorkload(tester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := certmanager.Generate(*workload, nil, nil)
	if err != nil {
...
func certificatesv1alpha1CertManagerNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &certificatesv1alpha1.CertManager{},
		sampleManifestFile: "../../config/samples/certificates_v1alpha1_certmanager.yaml",
		getChildrenFunc:    certificatesv1alpha1CertManagerChildrenFuncs,
		logSyntax:          "controllers.certificates.CertManager",
	}
}

func (tester *E2ETest) certificatesv1alpha1CertManagerTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_certificatesv1alpha1CertManager() {
	tester := certificatesv1alpha1CertManagerNewHarness("")
	tester.certificatesv1alpha1CertManagerTest(testSuite)
}
